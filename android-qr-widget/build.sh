...
# Clean
rm -rf "$OUT"
mkdir -p "$OUT/gen/$PACKAGE_PATH" "$OUT/classes" "$OUT/dex" "$OUT/aapt2"
...
echo "[1/8] Kompilace zdrojů (aapt2 compile)..."
aapt2 compile --dir res/ -o "$OUT/aapt2/compiled.zip"

# aapt2 link generuje R.java se stejnými ID jako resources.pb v AAB –
# tím se předchází neshodě ID mezi DEX kódem a resources z Play Store.
echo "[2/8] Linkování zdrojů a generování R.java (aapt2 link)..."
aapt2 link \
    -o "$OUT/resources.ap_" \
    -I "$ANDROID_JAR" \
...
echo "[3/8] Kompilace R.java (javac)..."
...
echo "[4/8] Kompilace Kotlin zdrojů (kotlinc)..."
kotlinc \
    -jvm-target 1.8 \
...
echo "[6/8] Sestavení a podepisování APK..."
...
# Linkování ve proto formátu – stejný compiled.zip zajišťuje shodná resource ID s R.java
