...
class ShowQRActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val amountFormatted = if (amountDouble == amountDouble.toLong().toDouble()) {
            "${amountDouble.toLong()} Kč"
        } else {
            "$amount Kč"
        }
        amountText.text = amountFormatted
        accountText.text = if (name.isNotEmpty()) "$iban  •  $name" else iban

        val shareText = if (name.isNotEmpty()) "$amountFormatted pro $name" else amountFormatted

        shareButton.setOnClickListener {
            shareQRCode(qrBitmap!!, shareText)
        }
    ...
    private fun shareQRCode(bitmap: Bitmap, shareText: String) {
        try {
